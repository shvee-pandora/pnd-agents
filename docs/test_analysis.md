...
- **NEW v2.2:** Mandatory interactive workflow (always asks questions before generating)
- **NEW v2.2:** Mandatory JIRA connection verification before all operations
- **NEW v2.2:** Workflow state tracking for multi-step interactions
...
## Mandatory Workflow & JIRA Verification (NEW v2.2)

### Mandatory Interactive Workflow

As of v2.2, the qAIn interactive workflow is **mandatory** for all invocations. The agent will always ask the workflow questions (hierarchy review, action selection) before generating test cases.

```python
# Configuration constant in agent.py
QAIN_WORKFLOW_MANDATORY = True
```

**Workflow State Tracking:**

The agent tracks workflow state using these constants:

| Constant | Purpose |
|----------|---------|
| `WORKFLOW_STATE_KEY` | Key for storing workflow state in context |
| `WORKFLOW_HIERARCHY_ANSWERED` | Tracks if hierarchy question was answered |
| `WORKFLOW_ACTION_ANSWERED` | Tracks if action question was answered |
| `WORKFLOW_HIERARCHY_CHOICE` | Stores user's hierarchy choice ("yes" or "no") |
| `WORKFLOW_ACTION_CHOICE` | Stores user's action choice ("recommend" or "create") |

### Mandatory JIRA Connection Verification

All JIRA operations now require mandatory connection verification before proceeding:

```python
from src.agents.test_analysis_design import verify_jira_connection

# Verify connection before any JIRA operation
connection_ok, connection_msg = verify_jira_connection(jira_client)
if not connection_ok:
    # Handle connection failure
    print(f"JIRA connection failed: {connection_msg}")
```

**When Verification is Performed:**
- Before `run_qain_workflow()`
- Before `create_jira_test_cases()`
- Before `run_jira_workflow()`

**Return Value Changes:**

Workflow results now include a `jira_connection_failed` flag:

```python
result = {
    "success": False,
    "error": "JIRA connection failed: ...",
    "jira_connection_failed": True,  # NEW: Indicates JIRA connection issue
    ...
}
```
...
```python
from src.agents.test_analysis_design import generate_test_cases
...
### 2. Workflow Context
```python
from src.agents.test_analysis_design import run
...
```python
from src.agents.test_analysis_design import run_jira_workflow
...
```python
from src.agents.test_analysis_design import (
    QAInWorkflowMode,
    run_qain_workflow,
...
```python
from src.agents.test_analysis_design import analyze_ticket_for_testing_types
...
|---------|------|---------|
| 2.2 | January 2026 | **Mandatory Workflow & JIRA Verification:** `QAIN_WORKFLOW_MANDATORY=True` makes interactive workflow mandatory, `verify_jira_connection()` function for mandatory JIRA verification, workflow state tracking constants, `jira_connection_failed` flag in results, `JiraTestCaseCreationResult.error_messages` field |
...
| Function | Purpose |
|----------|---------|
| `verify_jira_connection()` | Verify JIRA connection before any operations (mandatory) |

**New Constants in v2.2:**

| Constant | Value | Purpose |
|----------|-------|---------|
| `QAIN_WORKFLOW_MANDATORY` | `True` | Makes interactive workflow mandatory |
| `WORKFLOW_STATE_KEY` | `"qain_workflow_state"` | Key for workflow state in context |
| `WORKFLOW_HIERARCHY_ANSWERED` | `"hierarchy_question_answered"` | Track hierarchy question status |
| `WORKFLOW_ACTION_ANSWERED` | `"action_question_answered"` | Track action question status |
| `WORKFLOW_HIERARCHY_CHOICE` | `"hierarchy_choice"` | Store hierarchy choice ("yes"/"no") |
| `WORKFLOW_ACTION_CHOICE` | `"action_choice"` | Store action choice ("recommend"/"create") |

**Updated Data Structures in v2.2:**
...
class JiraTestCaseCreationResult:
    ...
    error_messages: List[str] = field(default_factory=list)  # NEW in v2.2
```
